...
@Composable
fun MeterWaterScaffold(navController: NavHostController){
    SampleScaffold(navController,"มิเตอร์น้ำ"){
        MeterWaterScreen(navController)
    }
}

@RequiresApi(Build.VERSION_CODES.O)
@Composable
fun MeterWaterScreen(navController: NavHostController) {
...
fun RoomWaterMeterCard(
    ...
    var previousMeter by remember(billData) { mutableIntStateOf(billData?.previousWaterUsed ?: 0) }
    var currentMeter by remember(billData) { mutableIntStateOf(billData?.currentWaterUsed ?: 0) }
    ...
    LaunchedEffect(save) {
        if (save && room.status == "ไม่ว่าง") {
            createWaterBillUtility(
                ...
                onResponse = {
                    Log.d("Bill", "Successfully created water bill for room ${room.roomId}")
                    onSaved()
                },
                onElse = { elseResponse ->
